...
@synthesize wantButton, shareButton, reportButton, comentaryTextField;
...
- (void)viewDidLoad {
    ...
    // seteamos el delegate del textField
    [[self comentaryTextField] setDelegate:self];
...
//    comentaryTextField.layer.masksToBounds = TRUE;
    ...
    // modificamos el highlighted background color del boton de notificacion de la celda
    [[self anotherStoresButton] setBackgroundImage:[CommonUtilities imageFromColor:[UIColor colorWithRed:230.0/255
                                                                                                   green:226.0/255
                                                                                                    blue:215.0/255
                                                                                                   alpha:0.4f]]
...
- (void)replyCommentButtonPressed:(id)sender
                       atIndexPath:(NSIndexPath *)indexPath
{
    [self presentPopUpTextView:indexPath];
}
...
- (void)presentPopUpTextView:(NSIndexPath *)indexPath
{
    // NOTE: maxCount = 0 to hide count
    YIPopupTextView *popupTextView = [[YIPopupTextView alloc] initWithPlaceHolder:@"Escribe tu comentario aquí"
                                                                         maxCount:300
                                                                      buttonStyle:YIPopupTextViewButtonStyleRightCancelAndDone
                                                                  doneButtonColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"header.png"]]];
    
    popupTextView.delegate = self;
    popupTextView.caretShiftGestureEnabled = YES;   // default = NO
    [popupTextView setTextColor:[UIColor colorWithRed:109./255 green:103./255 blue:98./255 alpha:1.0f]];
    [popupTextView setTag:[indexPath row]];
    
    popupTextView.layer.cornerRadius = 3.0f;
    popupTextView.layer.borderWidth = 1;
    popupTextView.layer.shadowColor = [UIColor blackColor].CGColor;
    popupTextView.layer.shadowRadius = 3.0f;
    popupTextView.layer.shadowOffset = CGSizeMake(0.0f, 0.0f);
    popupTextView.layer.shadowOpacity = 0.4f;

    
//    [popupTextView showInViewController:self]; // recommended, especially for iOS7
    [popupTextView showInViewController:[[[AppDelegate sharedAppdelegate] window] rootViewController]]; // recommended, especially for iOS7
}

- (void)popupTextView:(YIPopupTextView *)textView
   didDismissWithText:(NSString *)text
...
{    
    ...
    if ([text length] > 0 && !cancelled)
    {
        ...
        Comment *comment;
        NSString *formatedComment;
        if ([textView tag])
        {
            comment = [[self comments] objectAtIndex:[textView tag]];
            formatedComment = [NSString stringWithFormat:@"@%@ %@", [comment comment_author], text];
        } else {
            formatedComment = text;
        }
        
        [APIProvider createComment:formatedComment
                        forPicture:[[self objectDetail] idImage]
                 commentAuthorName:[[[AppDelegate sharedAppdelegate] currentUser] name]
                commentAuthorEmail:[[[AppDelegate sharedAppdelegate] currentUser] email]
                    withCompletion:^{
                        // Actualizo lista de comentarios de la prenda
                        [self loadComments];
                    }
...
{    
    ...
    // habilitamos el boton de responder un comentario
    [[cell replyCommentButton] setHidden:NO];
    ...
    // si el comentario es mio, oculto el boton responder
    if (commentIsMine)
        [[cell replyCommentButton] setHidden:YES];
    
    // Setear las imágenes y los textos
    [[cell nameLabel] setTitle:[@"  " stringByAppendingString:[comment comment_author]] forState:UIControlStateNormal];
