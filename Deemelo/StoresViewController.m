...
- (void)viewDidLoad
{
    ...
    [[[self markerLocationHelpLabel] layer] setCornerRadius:5.0f];
    [[[self markerLocationHelpLabel] layer] setBorderWidth:1.0f];
    [[[self markerLocationHelpLabel] layer] setShadowColor:[UIColor whiteColor].CGColor];
    [[[self markerLocationHelpLabel] layer] setShadowRadius:2.0f];
    [[[self markerLocationHelpLabel] layer] setShadowOffset:CGSizeMake(-1.0f, 1.0f)];
    [[[self markerLocationHelpLabel] layer] setShadowOpacity:2.0f];
    
    [[[self centerPositionButton] layer] setCornerRadius:4.0f];
    [[[self centerPositionButton] layer] setBorderWidth:0.5f];
    [[[self centerPositionButton] layer] setShadowColor:[UIColor whiteColor].CGColor];
    [[[self centerPositionButton] layer] setShadowRadius:5.0f];
    [[[self centerPositionButton] layer] setShadowOffset:CGSizeMake(-1.0f, 1.0f)];
    [[[self centerPositionButton] layer] setShadowOpacity:2.0f];
}
    ...
    [geoCoder reverseGeocodeLocation:currentLocation
                   completionHandler:^(NSArray *placemarks, NSError *error) {
                       if ([placemarks count] != 0) {
                           ...
                           [APIProvider getStoresFromLatitude:[currentLocation coordinate].latitude
...
{    
    lastStoreLocation = [[CLLocation alloc] initWithLatitude:[mapView centerCoordinate].latitude
                                                   longitude:[mapView centerCoordinate].longitude];
    
    // consultar nueva dirección
    CLGeocoder * geoCoder = [[CLGeocoder alloc] init];
    [geoCoder reverseGeocodeLocation:lastStoreLocation
                   completionHandler:^(NSArray *placemarks, NSError *error) {
                       if ([placemarks count] != 0)
                       {
                           NSMutableString *newAddress = [NSMutableString string];
                           
                           if ([[placemarks objectAtIndex:0] thoroughfare])
                               [newAddress appendString:[[placemarks objectAtIndex:0] thoroughfare]];

                           if ([[placemarks objectAtIndex:0] subThoroughfare])
                               [newAddress appendFormat:@" %@", [[placemarks objectAtIndex:0] subThoroughfare]];
...
- (void)presentAddStoreUI:(id)sender
{
    ...
    // mostramos el mensaje de ayuda del marcador
    [[self markerLocationHelpLabel] setHidden:NO];
    ...
    CGFloat x = ([[self mapView] bounds].size.width)/2;
    CGFloat y = ([[self mapView] bounds].size.height)/2;
    [self setStationaryMarkerIcon:[[UIImageView alloc] initWithFrame:CGRectMake(x-30, y-20, 60, 40)]];
    [[self stationaryMarkerIcon] setImage:[UIImage imageNamed:@"marker"]];
    [[self mapView] addSubview:[self stationaryMarkerIcon]];
}
...
- (void)dismissAddStoreUI:(id)sender
{
    ...
    // ocultamos el mensaje de ayuda para el usuario
    [[self markerLocationHelpLabel] setHidden:YES];
    
    // removemos el marcador estacionario del mapa
    [[self stationaryMarkerIcon] removeFromSuperview];
...
- (void)saveNewStore:(id)sender
{
    if ([[[self addingStoreNameTextField] text] length] < 1) {
        ...
        [newStore setLatitude:[NSString stringWithFormat:@"%f", [lastStoreLocation coordinate].latitude]];
        [newStore setLongitude:[NSString stringWithFormat:@"%f", [lastStoreLocation coordinate].longitude]];
...
- (IBAction)centerOnUserPosition:(id)sender
{
    // centramos el mapa en la posicion actual del usuario
