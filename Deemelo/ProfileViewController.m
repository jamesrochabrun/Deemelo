...
- (void) userUpdated:(NSNotification *)notice
{
    ...
    if ( [[self currentEmail] isEqualToString:[[appDelegate currentUser] email]] ) {
        ...
        [[self profileUrlButton] setTitle:[[appDelegate currentUser] url] forState:UIControlStateNormal];
...
- (void)viewDidLoad
{
    ...
    // Seteo el boton Editar para hacerlo redondeado y similar a otros botones
    [[[self configureButton] layer] setCornerRadius:7.0f];
    [[[self configureButton] layer] setBorderWidth:1];
    [[[self configureButton] layer] setMasksToBounds: TRUE];
    ...
    if ([[[self profileName] text] isEqualToString:@""]) {
        ...
        [APIProvider getProfileWithEmail:[self currentEmail]
                          withCompletion:^(User *profile) {
                              ...
                              // Seteo en la vista la URL del usuario
                              [[self profileUrlButton] setTitle:[profile url] forState:UIControlStateNormal];
...
- (IBAction)didSelectProfileUrl:(id)sender
{
    if ([sender isKindOfClass:[UIButton class]])
    {
        NSString *stringURL = [[(UIButton*)sender titleLabel] text];
        
        // modificamos la ruta si la url es de tipo facebook
        if ([[stringURL pathComponents][1] isEqualToString:@"www.facebook.com"])
        {
            NSMutableArray *pathComponents = [[stringURL pathComponents] mutableCopy];
            [pathComponents replaceObjectAtIndex:1 withObject:@"m.facebook.com"];
            stringURL = [NSString pathWithComponents:pathComponents];
        }
