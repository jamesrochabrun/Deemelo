...
{    
    ...
    if ([[self selectedStore] address])
    {
        // mostramos la direccion de la tienda sobre el mapa
        [[self storeAddressLabel] setHidden:NO];
        [[self storeAddressLabel] setText:[[self selectedStore] address]];

        // modificamos el estilo del view donde se muestra la direccion de la tienda
        [[[self storeAddressLabel] layer] setCornerRadius:12.0f];
        [[[self storeAddressLabel] layer] setShadowColor:[UIColor blackColor].CGColor];
        [[[self storeAddressLabel] layer] setShadowRadius:2.0f];
        [[[self storeAddressLabel] layer] setShadowOffset:CGSizeMake(-1.0f, 1.0f)];
        [[[self storeAddressLabel] layer] setShadowOpacity:0.2f];
    } else {
        [[self storeAddressLabel] setHidden:YES];
    }
}
